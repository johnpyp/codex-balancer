...
`x-codex-turn-state`, encrypted reasoning/compaction, or item references before
it sends work to a replacement account. Opaque account-bound input is never
stripped or moved. The replacement
...
Inference supports WebSockets and HTTP/SSE on `/v1/responses`, `/v1/guardian`,
and `/v1/guardian-classifier`. All other `/v1/*` paths are forwarded to the
configured upstream base, including HTTP requests and generic WebSocket upgrades.
The `/backend-api/codex/*` alias supports clients that select backend request
shapes by URL. See PROVIDER_COMPATIBILITY.md for the forwarding contract.
...
- A replacement request must omit `previous_response_id` and
  `x-codex-turn-state`, encrypted reasoning/compaction, and item references.
  Encrypted input remains bound to its original account; the relay rejects
  a move before forwarding that input.
...
- For a usage limit, mark the account spent. If the socket has a thread or
  ...
  metadata or either side of the handshake, no encrypted or referenced input
  is present, and another eligible account can
  ...
  portable history without an old response ID.
- Otherwise, forward the original usage-limit event and retire the socket.
  A fresh user turn or cold resume can choose another account only when its
  replay contains no account-bound state.
...
preservation of portable history, refusal to move a turn-state
