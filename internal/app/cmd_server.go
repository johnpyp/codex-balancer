...
For native ChatGPT login without a provider bearer override, run with -no-auth
on a literal loopback address, and use these top-level Codex settings:

  model_provider = "openai"
  openai_base_url = "http://127.0.0.1:8317/backend-api/codex"

This allows local processes to use the pool without a proxy key. The proxy
ignores incoming login credentials and replaces them with the selected pool
account. Codex's login file is never read or modified by the proxy.
...
func serverCmd(args []string) error {
	...
	insecure := fs.Bool("no-auth", false, "serve without a proxy key on loopback only; any local process can spend your quota")
	accessConfig := fs.String("client-access-config", "", "JSON file with allowed_cidrs; reloaded for every request, permits restricted non-loopback -no-auth")
	...
	if *insecure && *accessConfig == "" {
		if err := validateNoAuthAddress(*addr); err != nil {
			return err
		}
	}
	if *accessConfig != "" {
		if _, err := readClientAccess(*accessConfig); err != nil {
			return err
		}
	}
	...
	if *accessConfig != "" {
		handler = clientAccessHandler(*accessConfig, handler)
	}
	...
	if *insecure && *accessConfig == "" {
		if err := validateNoAuthListener(listener); err != nil {
			return err
		}
	}
