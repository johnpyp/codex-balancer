...
func TestWebSocketRequestPortableRecognizesAccountBoundState(t *testing.T) {
	tests := []struct {
		...
		{name: "encrypted reasoning", request: map[string]any{"input": []any{map[string]any{"type": "reasoning", "encrypted_content": "bound"}}}},
		{name: "encrypted compaction", request: map[string]any{"input": []any{map[string]any{"type": "compaction", "encrypted_content": "bound"}}}},
		{name: "item reference", request: map[string]any{"input": []any{map[string]any{"type": "item_reference", "id": "bound"}}}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
...
func TestWebSocketExhaustedOwnerRejectsEncryptedReplayAndRetainsOwner(t *testing.T) {
	var mu sync.Mutex
	requests := 0
	upstream := newWebSocketUpstream(t, func(account string, conn *websocket.Conn, _ websocketEnvelope) {
		mu.Lock()
	...
	writeWebSocketEvent(t, replayed, map[string]any{
		"type": "response.create",
		"input": []any{
			map[string]any{"type": "reasoning", "encrypted_content": "encrypted-history"},
		},
	})
	readCloseStatus(t, replayed, websocket.StatusTryAgainLater)
	replayed.CloseNow()
	...
	defer reconnected.CloseNow()
	completeWebSocketTurn(t, reconnected, map[string]any{"type": "response.create", "input": []any{}})
	if got := fmt.Sprint(upstream.RequestAccounts()); got != "[account-owner account-owner account-owner]" {
		t.Fatalf("request accounts = %s, want encrypted replay rejected before dispatch and origin owner retained", got)
	}
}
