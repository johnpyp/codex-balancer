...
// Check both the requested address and the effective listener. Socket activation
// can supply a different bind address than the command-line flag.
func validateNoAuthAddress(address string) error {
	host, _, err := net.SplitHostPort(address)
	if err != nil || net.ParseIP(host) == nil || !net.ParseIP(host).IsLoopback() {
		return fmt.Errorf("-no-auth requires a literal loopback address, such as 127.0.0.1:8317 or [::1]:8317")
	}
	return nil
}

func validateNoAuthListener(listener net.Listener) error {
	addr, ok := listener.Addr().(*net.TCPAddr)
	if !ok || !addr.IP.IsLoopback() {
		return fmt.Errorf("-no-auth requires the actual listener, including a systemd socket, to be bound to loopback")
	}
	return nil
}
