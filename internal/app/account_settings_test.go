...
func TestConnectAccountWithConfirmedTrainingDisabled(t *testing.T) {
	source := testAccount("account-a", 0).persisted()
	client := &http.Client{Transport: accountSettingsRoundTrip(func(*http.Request) (*http.Response, error) {
		t.Fatal("confirmed existing setting should not issue a settings request")
		return nil, nil
	})}
	ctx := context.WithValue(context.Background(), trainingAlreadyDisabledKey{}, true)
	account, err := connectAccount(ctx, client, tokenResponse{IDToken: source.IDToken, AccessToken: source.AccessToken, RefreshToken: source.RefreshToken})
	if err != nil || account == nil || account.id() != "account-a" {
		t.Fatalf("enrollment failed: %v", err)
	}
	if strings.Contains(trainingSettingNotice(ctx), "turns off") {
		t.Fatal("confirmation incorrectly claims to change the setting")
	}
}
