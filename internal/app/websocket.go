...
func websocketRequestPortable(event websocketEnvelope) bool {
	return strings.TrimSpace(event.PreviousResponseID) == "" &&
		...
		!accountBoundInput(event.Input)
}

// Reconnecting drops response IDs but does not make encrypted reasoning or
// compaction blobs portable between ChatGPT identities. Keep opaque state on
// its origin account instead of inviting invalid_encrypted_content failures.
func accountBoundInput(input json.RawMessage) bool {
	if len(input) == 0 {
		return false
	}
	...
	for _, item := range items {
		if value := item["encrypted_content"]; len(value) > 0 && string(value) != "null" && string(value) != `""` {
			return true
		}
		var kind string
