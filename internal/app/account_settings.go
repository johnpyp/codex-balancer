...
// Only the explicit CLI confirmation sets this context value. Web enrollment
// and ordinary logins still require a successful training settings update.
type trainingAlreadyDisabledKey struct{}

func trainingSettingNotice(ctx context.Context) string {
	if confirmed, _ := ctx.Value(trainingAlreadyDisabledKey{}).(bool); confirmed {
		return "Keeping the model training setting you confirmed is already off."
	}
	return "Adding this account turns off model training for it."
}
